 ...
 ******************************************************************************/
package com.leo.wifichecker.utils;
...
public class ExecTerminal {
	...
	public boolean checkSu(){
		...
		if (res.getStdOut().trim().equals("1")){
			LogEx.i(TAG, "^ got root!");
			return true;
		}

		LogEx.w(TAG, "^ could not get root.");
		return false;
	}

	public ExecResult exec(String cmd) {
		LogEx.d(TAG, "^ exec(): '" + cmd + "'");
		String stdOut = "";
		String stdErr = "";
		...
		try {
			...
			try {
				...
				LogEx.e(TAG, "^ exec() - IOException in sdtdOut Loop: " + e.getMessage());
			}
			...
			try {
				...
				LogEx.e(TAG, "^ exec() - IOException in stdErr Loop: " + e.getMessage());
			}

		} catch (IOException e) {
			LogEx.e(TAG, "^ exec() IOException: " + e.getMessage());

		} catch (InterruptedException e) {
			LogEx.e(TAG, "^ exec() InterruptedException: " + e.getMessage());
		}
	...
	public ExecResult execSu(String cmd) {
		...
		try {
			...
			try {
				...
				LogEx.e(TAG, "^ execSu() - IOException in sdtdOut Loop: " + e.getMessage());
			}
			...
			LogEx.e(TAG, "^ execSu() - IOException: " + e.getMessage());
		} catch (InterruptedException e) {
			LogEx.e(TAG, "^ execSu() - InterruptedException: " + e.getMessage());
		}
