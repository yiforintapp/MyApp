...
public class ImageGridActivity extends BaseActivity implements OnClickListener {
	 ...
	 * @param pathList
	 */
	private void batchRenameImageFile(ArrayList<PhotoItem> itemList,
			boolean isHide) {
		String newFileName;
		if (itemList != null && itemList.size() > 0) {
			if (isHide) {
				for (PhotoItem item : itemList) {
					newFileName = FileOperationUtil.getNameFromFilepath(item
							.getPath());
								...
								.makePath(
										FileOperationUtil
												.getDirPathFromFilepath(item
														.getPath()),
										newFileName), this);
						FileOperationUtil.deleteImageMediaEntry(item.getPath(),
								this);
											...
											FileOperationUtil.makePath(
													FileOperationUtil
															.getDirPathFromFilepath(item
																	.getPath()),
													newFileName), context);
							FileOperationUtil.deleteFileMediaEntry(
									item.getPath(), context);
							mPicturesList.remove(item);
							mAllListPath.remove(item.getPath());
						}
		...
		protected void onPostExecute(final Boolean isSuccess) {
			...
			if (!isSuccess) {
				Toast.makeText(ImageGridActivity.this,
						getString(R.string.app_hide_image_fail),
			...
			if (mPicturesList.size() > 0) {
				...
				// // animateReorder(minPos, mGridView.getLastVisiblePosition()
				// + 1 );
