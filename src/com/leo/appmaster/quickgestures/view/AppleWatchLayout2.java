...
public class AppleWatchLayout2 extends ViewGroup {
    ...
    public AppleWatchLayout2(Context context) {
        this(context, null);
    }

    public AppleWatchLayout2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AppleWatchLayout2(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void fillItems(List<BaseInfo> infos, boolean loadExtra) {
        ...
        AppleWatchLayout2.LayoutParams lp = null;
        for (int i = 0; i < infos.size(); i++) {
            info = (BaseInfo) infos.get(i);
            gestureItem = makeGestureItem();
            lp = new AppleWatchLayout2.LayoutParams(
                    mItemSize, mItemSize);
    ...
    public void replaceItems(GestureItemView fromView, GestureItemView toView) {
        AppleWatchLayout2.LayoutParams fromLP = (LayoutParams) fromView.getLayoutParams();
        AppleWatchLayout2.LayoutParams toLP = (LayoutParams) toView.getLayoutParams();
    ...
    public void squeezeItems(GestureItemView fromView, GestureItemView toView) {
        ...
        int from = ((AppleWatchLayout2.LayoutParams) fromView.getLayoutParams()).position;
        int to = ((AppleWatchLayout2.LayoutParams) toView.getLayoutParams()).position;

        boolean isForward = to > from;
        final GestureItemView[] hitViews = new GestureItemView[Math.abs(to - from) + 1];
        hitViews[0] = fromView;
        GestureItemView hitView;
        AppleWatchLayout2.LayoutParams hitLP;
        for (int i = 0; i < getChildCount(); i++) {
        ...
        mReorderAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                mRecodering = false;
                int lastPosition = ((AppleWatchLayout2.LayoutParams) hitViews[hitViews.length - 1]
                        .getLayoutParams()).position;
                int nextPosition;
                for (int i = hitViews.length - 1; i >= 0; i--) {
                    if (i == 0) {
                        ((AppleWatchLayout2.LayoutParams) hitViews[i].getLayoutParams()).position = lastPosition;
                        computeCenterItem(hitViews[i], lastPosition, false);
                    } else {
                        nextPosition = ((AppleWatchLayout2.LayoutParams) hitViews[i - 1]
                                .getLayoutParams()).position;
                        ((AppleWatchLayout2.LayoutParams) hitViews[i].getLayoutParams()).position = nextPosition;
            ...
            if (i < 4) {
                if (direction == 0) {// show from left-center
                    ...
                    tranAnimaX = ObjectAnimator
                            .ofFloat(targetItem, "translationX", -mTotalWidth, 0);
                    ...
                    tranAnimaY = ObjectAnimator
                            .ofFloat(targetItem, "translationY", mTotalHeight, 0);
