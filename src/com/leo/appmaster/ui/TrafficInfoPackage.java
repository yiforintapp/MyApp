    ...
    public List<TrafficsInfo> getRunningProcess(boolean loadIcon) {
        ...
        for (PackageInfo packInfo : packAgeList) {
            ...
            if(loadIcon) {
                ...
                Drawable mIconDrawable = AppUtil.getAppIcon(mPm, mPackageName);
                String mLabelName = AppUtil.getAppLabel(mPm, mPackageName);
            ...
            Cursor mFindCursor = null;
            try {
                mFindCursor = mContext.getContentResolver().query(Constants.APP_TRAFFIC_URI,
                        null,
            ...
            try {
                ...
                // 跨月清零
                mCursor = mContext.getContentResolver().query(Constants.APP_TRAFFIC_URI, null,
                        "year=? and month=? and uid=?", new String[] {
                                String.valueOf(nowYear), String.valueOf(nowMonth), String.valueOf(uid)
                        }, null);
                if (mCursor != null) {
            ...
            long rx = 0;
            long tx = 0;
            Cursor mFillCursor = null;
            try {
                // 重新获取，赋值
                mFillCursor = mContext.getContentResolver().query(Constants.APP_TRAFFIC_URI,
                        null,
                        "year=? and month=? and uid=?", new String[] {
                                String.valueOf(nowYear), String.valueOf(nowMonth), String.valueOf(uid)
                        }, null);
