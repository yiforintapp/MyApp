...
public class LockerTheme extends Activity {
	...
	private void initUI() {
		...
		sharedPreferences = getSharedPreferences("lockerTheme",
				Context.MODE_WORLD_WRITEABLE); // 私有数据
	}
	...
	protected void onCreate(Bundle arg0) {
		...
		/*
		 * //------------------默认 AppLockerThemeBean applockerTheme1=new
		 * AppLockerThemeBean();
		 * applockerTheme1.setThemeImage(R.drawable.select_all_press);
		 * applockerTheme1.setThemeName("默认主题"); String[] url1=new String[2];
		 * url1[1]=
		 * "http://down2.iinmobi.com/group1/M01/09/73/ooYBAFRh45eAGQVaACCBUDfiyfw938.apk"
		 * ; url1[0]="com.mah.calldetailscreen"; applockerTheme1.setUrl(url1);
		 * applockerTheme1.setPackageName("com.mah.calldetailscreen");
		 * applockerTheme1.setFlagName("默认"); mThemes.add(applockerTheme1);
		 */
		for (int i = 0; i < 10; i++) {
			AppLockerThemeBean applockerTheme = new AppLockerThemeBean();
			applockerTheme.setThemeImage(R.drawable.select_all_press);
			...
			// applockerTheme.setPackageName("com.mah.calldetailscreen");
			applockerTheme.setFlagName("在线");
			applockerTheme.setPackageName("com.leo.theme.theme" + i);
		...
		listTheme.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Toast.makeText(LockerTheme.this, "在线主题" + (arg2 + 1),
						Toast.LENGTH_SHORT).show();
				/*
				 * itemTheme=(AppLockerThemeBean) arg0.getItemAtPosition(arg2);
				 * String[] urls=itemTheme.getUrl(); showAlarmDialog(null,null);
				 * if
				 * (mThemes.get(arg2).getFlagName().equals((String)getResources
				 * ().getText(R.string.onlinetheme))){ for (int i = 0; i <
				 * urls.length; i++) { boolean flag =
				 * AppwallHttpUtil.isHttpUrl(urls[i]); if (flagGp) { if (!flag)
				 * { AppwallHttpUtil.requestGp(LockerTheme.this,urls[i]); break;
				 * } } else { AppwallHttpUtil.requestUrl(LockerTheme.this,
				 * urls[i]); break; } } }else
				 * if(mThemes.get(arg2).getFlagName().
				 * equals((String)getResources().getText(R.string.localtheme))){
				 * 
				 * 卸载主题
				 * 
				 * Uri uninstallUri = Uri.fromParts("package", "xxx", null);
				 * Intent intent = new Intent(Intent.ACTION_DELETE,
				 * uninstallUri); startActivity(intent); }
				 */
		...
		for (int i = 0; i < pkgInfos.size(); i++) {
			...
			if (flagPackge) {
				for (int j = 0; j < mThemes.size(); j++) {
					if (mThemes.get(j).getPackageName()
							.equals(pkgInfos.get(i).getPkg())) {
		...
		if (mAlarmDialog == null) {
			...
			mAlarmDialog.setOnClickListener(new OnDiaogClickListener() {
				...
				public void onClick(int which) {
					if (which == 0) {
						Toast.makeText(LockerTheme.this, "应用",
								Toast.LENGTH_SHORT).show();
						...
						Toast.makeText(LockerTheme.this, "卸载",
								Toast.LENGTH_SHORT).show();

					} else if (which == 2) {
						Toast.makeText(LockerTheme.this, "取消",
								Toast.LENGTH_SHORT).show();
					}
				}
			});
