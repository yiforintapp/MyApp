...
public class LockerTheme extends Activity {
	...
	public void showAlarmDialog() {
		View inflater = LayoutInflater.from(this).inflate(
		...
		uninstall.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// 卸载主题
				Uri uri = Uri.fromParts("package", itemTheme.getPackageName(),
						null);
	...
	public void getTeme() {
		...
		for (int i = 0; i < localThemes.size(); i++) {
			...
			if (flag) {
				...
				int themeres = saveContext.getResources().getIdentifier(
						"theme", "drawable", saveContext.getPackageName());
				if (themeres > 0) {
					tempTheme.setThemeImage(saveContext.getResources()
							.getDrawable(
									saveContext.getResources().getIdentifier(
											"theme", "drawable",
	...
	public OnItemClickListener item = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				long arg3) {
			// Toast.makeText(LockerTheme.this,
			// "在线主题"+(arg2+1),Toast.LENGTH_SHORT).show();
