...
public class HolographicOutlineHelper {
	...
	private static final MaskFilter sCoarseClipTable = TableMaskFilter
			.CreateClipTable(0, 200);

	private int[] mTempOffset = new int[2];

	private static HolographicOutlineHelper instance;

	public synchronized static HolographicOutlineHelper getInstance() {
		if (instance == null) {
	...
	public void applyOuterBlur(Bitmap bitmap, Canvas canvas, int color) {
		...
		canvas.drawBitmap(glow, mTempOffset[0], mTempOffset[1],
				mHolographicPaint);
		glow.recycle();
	}
	...
	void applyExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas,
		...
		applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color,
				outlineColor, mAlphaClipPaint, thickness);
	}

	void applyExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas,
			int color, int outlineColor, Paint alphaClipPaint, int thickness) {
	...
	public void applyExtraThickExpensiveOutlineWithBlur(Bitmap srcDst,
			Canvas srcDstCanvas, int color, int outlineColor) {
		applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color,
				outlineColor, EXTRA_THICK);
	}

	public void applyThickExpensiveOutlineWithBlur(Bitmap srcDst,
			Canvas srcDstCanvas, int color, int outlineColor) {
		applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color,
				outlineColor, THICK);
	}

	public void applyMediumExpensiveOutlineWithBlur(Bitmap srcDst,
			Canvas srcDstCanvas, int color, int outlineColor,
			Paint alphaClipPaint) {
		applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color,
				outlineColor, alphaClipPaint, MEDIUM);
	}
