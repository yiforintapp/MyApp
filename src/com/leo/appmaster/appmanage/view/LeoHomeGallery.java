 ...
 * 首页gallery
 */
public class LeoHomeGallery extends ViewGroup {
	/** touch状态集 */
	...
	/** 弹性比，基于单屏宽度 */
	private static final float ELASTIC_RATIO = 0.6f;
	/** 速度阈值 */
	...
	private GalleryMoveObserver mMoveObserver;

	public interface GalleryMoveObserver {
		public void onGalleryStartMove();

		public void onGalleryEndMove();
	}

	/**
	 * Constructor
	 */
	public LeoHomeGallery(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, TYPE_HOME);
	}
	 ...
	 *            context
	 */
	public LeoHomeGallery(Context aContext, int type) {
		super(aContext);
		init(aContext, type);
	}
	...
	public void setMoveListener(GalleryMoveObserver moveObserver) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		this.setMeasuredDimension(mTotalWidth,
	...
	public void drawDividerLine(Canvas aCanvas) {
		...
		// 计算分割线
		drawDividerLine(aCanvas, mDividerLinePaint, 0, COLOR_CROSS_LINE,
				mDividerStrokeWidth);
	 ...
	 *            线宽度
	 */
	public void drawDividerLine(Canvas aCanvas, Paint aPaint, int aLineOffset,
			int aLineColor, int aLineWidth) {
		// 变量
		...
		for (int i = 0; i < childCount; i++) {
			...
			for (int j = 0; j < aLineWidth; j++) {
				aCanvas.drawLine(offset + j, 0, offset + j,
						getMeasuredHeight(), aPaint);
			}
		}
	}
	...
	public void snapToScreen(int aWhichScreen, boolean aCycle) {
		...
		if (mType == TYPE_MENU) {
			snapToScreenWithDuration(aWhichScreen, 400, aCycle);
		} else {
			snapToScreenWithDuration(aWhichScreen, 400, aCycle);
		}
	}
	 ...
	 *            切换动作的时长
	 */
	public void snapToScreenWithDuration(int aWhichScreen, int aDuration,
			boolean aCycle) {
		aWhichScreen = aWhichScreen < 0 ? 0
				: aWhichScreen > (getChildCount() - 1) ? (getChildCount() - 1)
						: aWhichScreen;
		...
		if (mListener != null) {
			mListener
					.onGalleryScreenChanged(getChildAt(mCurScreen), mCurScreen);
		}
		...
		postDelayed(new Runnable() {
			...
			public void run() {
				if (mListener != null) {
					mListener.onGalleryScreenChangeComplete(
							getChildAt(mCurScreen), mCurScreen);
				}
			}
		}, 100); // SUPPRESS CHECKSTYLE
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if ((action == MotionEvent.ACTION_MOVE)
				&& (mTouchState != TouchState.REST) && !isXLocked) {
			return true;
		}
		...
		case MotionEvent.ACTION_DOWN:
			...
			mTouchState = mScroller.isFinished() ? TouchState.REST
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (Math.abs(x - mDownMotionX) >= thresholdX) {

				if (mListener != null && deltaX != 0) {
					mListener.onXChange(deltaX);
				}

				if (mMoveObserver != null && deltaX != 0) {
					mMoveObserver.onGalleryStartMove();
				}
