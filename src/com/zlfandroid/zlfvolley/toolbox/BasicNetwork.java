...
import com.zlfandroid.zlfvolley.NetworkError;
import com.zlfandroid.zlfvolley.ServerError;
 ...
 */
public class BasicNetwork implements com.zlfandroid.zlfvolley.Network {
	protected static final boolean DEBUG = com.zlfandroid.zlfvolley.VolleyLog.DEBUG;
	...
	@Override
	public com.zlfandroid.zlfvolley.NetworkResponse performRequest(com.zlfandroid.zlfvolley.Request<?> request)
			throws com.zlfandroid.zlfvolley.VolleyError {
		...
		while (true) {
			...
			try {
				...
				if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
					...
					com.zlfandroid.zlfvolley.Cache.Entry entry = request.getCacheEntry();
					if (entry == null) {
						return new com.zlfandroid.zlfvolley.NetworkResponse(HttpStatus.SC_NOT_MODIFIED,
...
//	                }
				...
				return new com.zlfandroid.zlfvolley.NetworkResponse(statusCode, responseContents,
						responseHeaders, false);
			} catch (SocketTimeoutException e) {
				attemptRetryOnException("socket", request, new com.zlfandroid.zlfvolley.TimeoutError());
			} catch (ConnectTimeoutException e) {
				attemptRetryOnException("connection", request,
						new com.zlfandroid.zlfvolley.TimeoutError());
			} catch (MalformedURLException e) {
				throw new RuntimeException("Bad URL " + request.getUrl(), e);
			} catch (IOException e) {
				int statusCode = 0;
				com.zlfandroid.zlfvolley.NetworkResponse networkResponse = null;
				if (httpResponse != null) {
					statusCode = httpResponse.getStatusLine().getStatusCode();
				} else {
					throw new com.zlfandroid.zlfvolley.NoConnectionError(e);
				}
				com.zlfandroid.zlfvolley.VolleyLog.e("Unexpected response code %d for %s", statusCode,
						request.getUrl());
				if (responseContents != null) {
					networkResponse = new com.zlfandroid.zlfvolley.NetworkResponse(statusCode,
							responseContents, responseHeaders, false);
					if (statusCode == HttpStatus.SC_UNAUTHORIZED
							|| statusCode == HttpStatus.SC_FORBIDDEN) {
						attemptRetryOnException("auth", request,
								new com.zlfandroid.zlfvolley.AuthFailureError(networkResponse));
					} else {
	 ...
	 */
	private void logSlowRequests(long requestLifetime, com.zlfandroid.zlfvolley.Request<?> request,
			byte[] responseContents, StatusLine statusLine) {
		if (DEBUG || requestLifetime > SLOW_REQUEST_THRESHOLD_MS) {
			com.zlfandroid.zlfvolley.VolleyLog
	...
	private static void attemptRetryOnException(String logPrefix,
												com.zlfandroid.zlfvolley.Request<?> request, com.zlfandroid.zlfvolley.VolleyError exception) throws com.zlfandroid.zlfvolley.VolleyError {
		com.zlfandroid.zlfvolley.RetryPolicy retryPolicy = request.getRetryPolicy();
		int oldTimeout = request.getTimeoutMs();

		try {
			retryPolicy.retry(exception);
		} catch (com.zlfandroid.zlfvolley.VolleyError e) {
	...
	private void addCacheHeaders(Map<String, String> headers, com.zlfandroid.zlfvolley.Cache.Entry entry) {
	...
	protected void logError(String what, String url, long start) {
		...
		com.zlfandroid.zlfvolley.VolleyLog.v("HTTP ERROR(%s) %d ms to fetch %s", what, (now - start),
				url);
	}
	...
	private byte[] entityToBytes(HttpEntity entity, boolean supportGzip, boolean supportAes) throws IOException,
		...
		try {
			...
			try {
				...
				// entity in
				// an invalid state.
				com.zlfandroid.zlfvolley.VolleyLog.v("Error occured when calling consumingContent");
			}
