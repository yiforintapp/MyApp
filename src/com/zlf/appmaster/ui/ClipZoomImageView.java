...
public class ClipZoomImageView extends ImageView implements
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		int frameHeight = getHeight() - mVerticalPadding * 2;

		// 大图
		if (drawableW > frameSize && drawableH < frameHeight) {
			scale = 1.0f * frameHeight / drawableH;
		} else if (drawableH > frameHeight && drawableW < frameSize) {
			scale = 1.0f * frameSize / drawableW;
		} else if (drawableW > frameSize && drawableH > frameHeight) {
			float scaleW = frameSize * 1.0f / drawableW;
			float scaleH = frameHeight * 1.0f / drawableH;
		...
		if (drawableW < frameSize && drawableH > frameHeight) {
			...
			float scaleH = 1.0f * frameHeight / drawableH;
			scale = Math.max(scaleW, scaleH);
		}
	...
	public Bitmap clip() {
		...
		return Bitmap.createBitmap(bitmap, mHorizontalPadding,
				...
				/*getWidth() - 2 * mHorizontalPadding*/getHeight() - 2 * getHVerticalPadding());
	}
	...
	private int getHVerticalPadding() {
		return /*(getHeight() - (getWidth() - 2 * mHorizontalPadding)) / 2;*/mVerticalPadding;
	}
